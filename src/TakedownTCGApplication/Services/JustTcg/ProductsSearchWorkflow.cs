...
public sealed class ProductsSearchWorkflow : IProductsSearchWorkflow
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ProductsSearchOperationResult result = model.Api.Equals(PokemonTcgApi, StringComparison.OrdinalIgnoreCase)
                ? await SearchPokemonAsync(model, userName, cancellationToken)
                : await SearchJustTcgAsync(model, userName, cancellationToken);
    ...
    {
        return api.Equals(PokemonTcgApi, StringComparison.OrdinalIgnoreCase)
            ? "Pokemon API currently supports card search only."
